# Headroom + OpenCode，更省的编程助手 — 技术交付报告

> **历史材料。** Headroom 不在当前 `project.config.json` 正式范围；本文不作为部署、安全或云测证据。
...
本方案面向使用 OpenCode 进行 AI 辅助编程的开发者，通过部署 [Headroom](https://github.com/chopratejas/headroom) 上下文压缩代理，在华为云 Flexus X 实例上一键搭建"OpenCode + Headroom 代理 + MaaS API"全链路编程环境，实现 **60-95% 的 Token 成本压缩**，答案质量不降。

### 1.2 应用场景

| 场景 | 说明 | 压缩率 |
|------|------|--------|
| **日常开发编程** | 开发者使用 OpenCode 进行代码生成、重构、调试，Headroom 自动压缩上下文，降低每轮对话 Token 消耗 | 60-80% |
...
| 组件 | 说明 | 端口 | 版本 |
|------|------|------|------|
| **OpenCode CLI** | 开源 AI 编程助手，通过 `opencode.json` 配置连接本地代理 | — | 最新版 (npm) |
...
|-----------|---------|------|------|------------|
| 虚拟私有云 VPC | headroom-opencode-vpc | 172.16.0.0/16，华北-北京四 | 1 | 0.00 元 |
| 子网 Subnet | headroom-opencode-subnet | 172.16.1.0/24 | 1 | 0.00 元 |
| 安全组 SecurityGroup | headroom-opencode-sg | 放行 ICMP、22、8787 端口 | 1 | 0.00 元 |
| Flexus 云服务器 X 实例 | headroom-opencode-ecs | x1.2u.4g，Ubuntu 24.04，40GB 高 IO | 1 | ≈233.60 元 |
| 弹性公网 IP EIP | headroom-opencode-eip | 动态 BGP，300Mbit/s，按流量计费 0.80 元/GB | 1 | ≈44 元（视流量） |
| **合计** | — | — | — | **≈277.60 元 + 流量费** |
...
| OpenCode | 最新版 (npm) | `npm i -g opencode-ai@latest` | AI 编程工具 |
| Python | ≥ 3.10 | 系统预装 | Headroom 运行时 |
| Node.js | ≥ 18.x | 通过 nvm 安装 | OpenCode 运行时 |
...
### 4.4 配置 MaaS 并启动 OpenCode

```bash
# 配置 Headroom 代理环境变量（替换 your-maas-api-key 为实际值）
cat >> /root/.bashrc << 'EOF'
...
在 OpenCode 中发送任意消息（如"hello"），然后另开终端检查：
...
| OpenCode 启动 | 进入交互模式 | 执行 `opencode` 命令 |
...
### 8.4 OpenCode 发消息但 stats 没变化

**原因：** `opencode.json` 中的 `base_url` 未正确配置，请求没经过代理。

**排查：**
```bash
# 检查 OpenCode 配置
cat /root/opencode.json
# 确认 base_url 为 http://localhost:8787
...
cat /root/opencode.json               # OpenCode 配置，确认 base_url
...
| **OpenCode** | 开源 AI 编程助手，支持代码生成、重构、调试等 AI 辅助编程能力，通过 `opencode.json` 配置文件管理连接 |
...
- [OpenCode GitHub](https://github.com/opencode-ai/opencode) — OpenCode 官方仓库
...
| 版本 | 日期 | 变更说明 |
|------|------|---------|
| v1.0 | 2026-06-12 | 初始版本，基于 Headroom + OpenCode 方案输出 |
...
| 发布日期 | 版本 | 修订内容 |
|---------|------|---------|
| 2026-06-12 | v1.0 | 初始版本，基于 Headroom + OpenCode 全链路方案输出 |
