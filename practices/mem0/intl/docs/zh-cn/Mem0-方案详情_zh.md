...
> 文档类型：华为云解决方案实践方案详情
> 适用范围：`intl` / 多区域 / `standard`

## 1. 方案概述

本方案在国际站多个区域（ap-southeast-1、ap-southeast-2、ap-southeast-3、af-south-1、af-north-1、la-north-2、sa-brazil-1、tr-west-1）的单个 VPC 内部署 Mem0。单台 Ubuntu 24.04 ECS 通过 Docker Compose 运行 Mem0 API、PostgreSQL（pgvector）和 Mem0 Dashboard；EIP 绑定 ECS，Mem0 API 通过公网 TCP `8888` 访问，Dashboard 通过公网 TCP `3000` 访问。LLM 与 Embedding 服务由客户在部署后通过 Dashboard 或本地配置文件配置。
...
EIP
   │
Ubuntu 24.04 ECS（1 台，c7n.2xlarge.2，100 GiB GPSSD）
  └─ Docker Compose
