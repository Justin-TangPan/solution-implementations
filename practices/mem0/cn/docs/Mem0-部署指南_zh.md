...
Mem0 是一个 AI Agent 通用记忆层，为 AI 应用提供持久化、可学习的记忆能力。本方案通过华为云资源编排服务 RFS，在华北-北京四创建 1 个资源栈，部署 Mem0 自托管服务。
...
- **一键部署：** 自动安装 Docker、拉取镜像、启动 Mem0 全栈服务。
...
- 部署前须确认账号状态、服务权限、资源配额与目标规格在华北-北京四可售。
- 首次部署时需要镜像构建过程，约需 10-15 分钟。
...
| ECS | `x1.4u.8g`，Ubuntu 24.04 | 1 | 按需实例时长 |
...
2. 确认在华北-北京四具备 RFS、VPC、EIP、ECS 等资源权限。
...
curl -X POST "$MEM0_API:8888/memories" \
  ...
  -d '{"messages": [{"role": "user", "content": "我喜欢徒步旅行"}], "user_id": "test-user"}'
```
...
curl -X POST "$MEM0_API:8888/search" \
  -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "喜欢什么户外活动", "user_id": "test-user"}'
```

预期返回包含"徒步旅行"的相关记忆。

#### 使用 API Key 初始化 Mem0 客户端
...
memory = Memory.from_config({
    ...
    "vector_store": {
        ...
        "config": {
            ...
            "dbname": "postgres",
            "user": "postgres",
            "password": "从 /opt/mem0/.env 获取 POSTGRES_PASSWORD",
...
# 添加记忆
memory.add("我喜欢编程", user_id="dev-user", agent_id="assistant")

# 搜索记忆
results = memory.search("爱好", user_id="dev-user")
print(results)
```
...
- [Mem0 研究论文](https://arxiv.org/abs/2504.19413)（Building Production-Ready AI Agents with Scalable Long-Term Memory）
...
| 发布日期 | 修订记录 |
|---|---|
| 2026-08-06 | 初始版本，基于 `deploying-mem0.tf` 实现 cn-north-4 标准版。 |
