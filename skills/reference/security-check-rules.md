...
> `sac-quality` 按语义和实际可利用性审计完整部署链路。搜索命令只能帮助定位，不能代替上下文判断或作为通过依据。
...
| SEC-003 | high | 管理端口、数据库、缓存、消息系统等不得向不受信网络宽泛开放 | CIDR、端口、实际监听地址、认证和架构方案共同判断 |
...
| SEC-008 | medium | 持久化数据、备份和日志不得产生超出方案的数据暴露或不可恢复风险 | 卷权限、公开存储、敏感日志、备份保护、删除与恢复路径 |
